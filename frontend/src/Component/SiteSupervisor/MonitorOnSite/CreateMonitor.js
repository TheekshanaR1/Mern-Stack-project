...
function CreateMonitor() {
  ...
  const [nextProjectId, setNextProjectId] = useState("Loading...");

  // Fetch next Project ID
  useEffect(() => {
    const fetchNextProjectId = async () => {
      try {
        const response = await axios.get("http://localhost:5000/Monitoring/next-project-id");
        if (response.data.success) {
          setNextProjectId(response.data.nextProjectId);
        }
      } catch (error) {
        console.error("Error fetching next Project ID:", error);
        setNextProjectId("Error loading ID");
      }
    };
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <small style={{ color: '#666', fontSize: '12px' }}>
                  This ID will be automatically assigned to your project
                </small>
              </div>
